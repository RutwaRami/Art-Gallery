...
def home(request) :
    if not request.user.is_authenticated :
        ...
        arts=art.objects.filter(available=1)[0:10]
...
def search(request) :
    ...
    if results.exists() :
        arts=results
        return render(request,'search.html',{'arts':arts,'cat':category})   #write code to show results
    else :
        arts=art.objects.filter(available=1)[0:10]
        msg="Sorry! No product available for your search:("
...
def remove(request):
    ...
    if results.exists() :
        ...
        for result in results :
            ...
            obj=art.objects.get(artname=name)
            arts.append(obj)
        msg="Succesfully removed from cart"    
